...
output=struct('mean_spontaneous', [],'mean_aCSF', [],'mean_content', [],'median_spontaneous',[],'median_aCSF',[],'median_content',[],'mean_all_aCSF',[],'mean_all_content',[],'median_all_aCSF',[],'median_all_content',[],'mean_delay_content',[],'mean_timetopeak_content',[],'median_delay_content',[],'median_timetopeak_content',[],'hypothesis_ttest',[],'p_value_ttest',[],'tstats_ttest',[]);
...
%Figure of the peak amplitudes aCSF VS content all puffs

figure(5);
x_amplis_afterallpuff_aCSF(1:1:size(amplis_afterallpuff_aCSF,1),1)=1;
x_amplis_afterallpuff_content(1:1:size(amplis_afterallpuff_content,1),1)=1.5;
sz = 75;
for i = 1:size(amplis_afterallpuff_aCSF,2)
    (scatter(x_amplis_afterallpuff_aCSF,amplis_afterallpuff_aCSF(:,i),sz,'k','filled','MarkerFaceColor','b','MarkerEdgeColor','b','MarkerFaceAlpha',.5,'MarkerEdgeAlpha',.5));
    hold on
end
hold on
for i = 1:size(amplis_afterallpuff_content,2)
    (scatter(x_amplis_afterallpuff_content,amplis_afterallpuff_content(:,i),sz,'k','filled','MarkerFaceColor','b','MarkerEdgeColor','b','MarkerFaceAlpha',.5,'MarkerEdgeAlpha',.5));
    hold on
end
hold on
xlim([0.5 2]);
xticks([1 1.5]);
title('Maximum DF/F aCSF VS content all puffs','FontSize',18);
ylim([-15 100]);
yticks(0:20:100);
set(gca, 'xticklabel', {'aCSF', 'Content'}); axis square;
ylabel('%DF/F');
hold on
for i = 1:size(amplis_afterallpuff_aCSF,1)
    for j = 1:size(amplis_afterallpuff_aCSF,2)
        line([1,1.5] , [amplis_afterallpuff_aCSF(i,j),amplis_afterallpuff_content(i,j)],'Color',[0.7 0.7 0.7]);
    end
end
mean_all_aCSF = [mean(amplis_afterallpuff_aCSF(:),'omitnan')];
mean_all_content = [mean(amplis_afterallpuff_content(:),'omitnan')];
median_all_aCSF = [median(amplis_afterallpuff_aCSF(:),'omitnan')];
median_all_content = [median(amplis_afterallpuff_content(:),'omitnan')];
line([1,1.5], [mean_all_aCSF(1,1), mean_all_content(1,1)], 'Color', 'r', 'LineWidth', 3);
line([1,1.5], [median_all_aCSF(1,1), median_all_content(1,1)], 'Color', 'k', 'LineWidth', 3);
hold off;
vccd = strcat(folder_content, 'Maximum DFF aCSF VS content all puffs');
saveas(gcf,vccd,'fig');

output.mean_all_aCSF=mean_all_aCSF;
output.mean_all_content=mean_all_content;
output.median_all_aCSF=median_all_aCSF;
output.median_all_content=median_all_content;

%%%%% Statistics %%%%%

[h,p,stats] = ttest(amplis_afterallpuff_aCSF(:),amplis_afterallpuff_content(:));
